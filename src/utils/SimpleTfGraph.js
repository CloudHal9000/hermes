...
export class SimpleTfGraph {
  constructor(ros) {
    this.graph = {}; // Armazena a árvore: frame -> [{to, t, q}]
    
    // Assina os tópicos de transformação
  ...
  _onTf(msg) {
    ...
    msg.transforms.forEach((tf) => {
      ...
      // Adiciona aresta normal e invertida para permitir busca bidirecional
  ...
  // Busca o caminho de transformações entre dois frames (BFS)
  lookupTransform(source, target, maxDepth = 15) {
    ...
    while (queue.length > 0) {
      ...
      for (let i = 0; i < edges.length; i++) {
        ...
        // Otimização: Se a fila ficar muito grande, aborta
        if (queue.length > 100) break; 
      }
    }
    return null; // Não encontrou conexão
  }

  // --- MATEMÁTICA DE QUATERNIONS E VETORES ---
