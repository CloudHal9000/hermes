export class AMCLHelper {
  ...
  setGoal(x, y, yawDeg) {
    ...
    const qz = Math.sin(yawRad / 2);
    const qw = Math.cos(yawRad / 2);
    const topic = new window.ROSLIB.Topic({ ros: this.ros, name: '/goal_pose', messageType: 'geometry_msgs/msg/PoseStamped' });
    const msg = new window.ROSLIB.Message({
  ...
  reinitializeGlobalLocalization() {
    ...
    const service = new window.ROSLIB.Service({ ros: this.ros, name: '/reinitialize_global_localization', serviceType: 'std_srvs/srv/Empty' });
