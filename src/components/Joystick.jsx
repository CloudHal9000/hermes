...
export function Joystick({ ros }) {
  ...
  // Refs de controle (Mantêm valor sem renderizar)
  ...
  // Ref para evitar loop de renderização
  ...
  // --- 3. CÁLCULO DE VELOCIDADE (Loop Lógico) ---
  const computeVelocity = useCallback(() => {
    ...
    // Atualiza Estado Visual APENAS se mudou (Evita destruir o componente)
  ...
  // --- 5. CRIAÇÃO DO JOYSTICK (Visual) ---
  // Importante: NÃO depende de 'inputType'. Assim ele não se destrói quando você aperta um botão.
  useEffect(() => {
  ...
  }, [ros, isAllowed, computeVelocity]); // <--- Dependências limpas!

  // Se não permitido, não renderiza nada
